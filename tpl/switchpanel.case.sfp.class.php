<?php
	class switchpanel_case_sfp{
		public static function getSvg( $oCase, $iX, $iY, $opt ){
			...
			// SFP port design - rectangular slot with metallic contacts
			$sSvg .= '<rect x="'.$iX.'" y="'.$iY.'" width="'.$opt[ 'elementWidth' ].'" height="'.$opt[ 'elementHeight' ].'" fill="'.$sColor.'"/>
				...
				<rect x="'.( $iX + ( $opt[ 'elementWidth' ] / 5 ) ).'" y="'.( $iY + ( $opt[ 'elementHeight' ] / 1.6 ) ).'" width="'.( $opt[ 'elementWidth' ] / 1.67 ).'" height="'.( $opt[ 'elementHeight' ] / 8 ).'" fill="#C0C0C0"/>';
