<?php
	class switchpanel_case_usb{
		public static function getSvg( $oCase, $iX, $iY, $opt ){
			...
			// USB port design - rectangular port with USB symbol
			$sSvg .= '<rect x="'.$iX.'" y="'.$iY.'" width="'.$opt[ 'elementWidth' ].'" height="'.$opt[ 'elementHeight' ].'" fill="'.$sColor.'"/>
				...
				<rect x="'.( $iX + ( $opt[ 'elementWidth' ] / 4 ) ).'" y="'.( $iY + ( $opt[ 'elementHeight' ] / 1.8 ) ).'" width="'.( $opt[ 'elementWidth' ] / 2 ).'" height="'.( $opt[ 'elementHeight' ] / 5 ).'" fill="#000000" stroke-width="1" stroke="#333333" rx="1"/>
				<rect x="'.( $iX + ( $opt[ 'elementWidth' ] / 3.5 ) ).'" y="'.( $iY + ( $opt[ 'elementHeight' ] / 1.65 ) ).'" width="'.( $opt[ 'elementWidth' ] / 2.8 ).'" height="'.( $opt[ 'elementHeight' ] / 12 ).'" fill="#FFFFFF"/>
				<circle cx="'.( $iX + ( $opt[ 'elementWidth' ] / 2 ) ).'" cy="'.( $iY + ( $opt[ 'elementHeight' ] / 1.3 ) ).'" r="2" fill="#0078D4"/>
				<text x="'.( $iX + ( $opt[ 'elementWidth' ] / 2 ) ).'" y="'.( $iY + ( $opt[ 'elementHeight' ] / 1.1 ) ).'" text-anchor="middle" font-family="sans-serif" font-size="'.( $opt[ 'elementWidth' ] * 0.15 ).'" fill="#0078D4" font-weight="bold">USB</text>';
